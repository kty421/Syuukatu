...
- `EXPO_PUBLIC_API_BASE_URL`: iOS / Android から呼び出す API のベース URL。Production では Vercel 本番URL、Preview検証時のみPreview URLを設定します。Web本番は同一オリジンの `/api` を使えるため空でも動きます。
- `EXPO_PUBLIC_SUPABASE_URL`: Supabase Project URL。クライアントに公開されます。
- `EXPO_PUBLIC_SUPABASE_ANON_KEY`: Supabase anon key。クライアントに公開されます。
...
- `SUPABASE_URL`: Vercel Functions 用の Supabase Project URL。
- `SUPABASE_ANON_KEY`: Vercel Functions 用の Supabase anon key。
...
Preview Deploy は Pull Request / ブランチごとの確認用、Production Deploy は本番公開用です。現状は `main` ブランチを Production に接続し、必要に応じて `develop` や feature branch を Preview として使う運用を推奨します。iOS / Android からPreview APIを検証する場合も、Preview URLがDeployment Protectionで保護されているとAPI通信できないため、保護解除済みのPreview URLだけを `EXPO_PUBLIC_API_BASE_URL` に設定してください。
