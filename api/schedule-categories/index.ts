...
export default async function handler(
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (error) {
        sendJson(res, 400, { error: '色カテゴリの読み込みに失敗しました。' });
        return;
      }

      sendJson(res, 200, {
        categories: ((data ?? []) as ScheduleCategoryRow[]).map(
          fromScheduleCategoryRow
        )
      });
      return;
    ...
    if (req.method === 'DELETE') {
      ...
      if (!id) {
        sendJson(res, 400, { error: 'カテゴリIDが指定されていません。' });
        return;
      }
      ...
      if (scheduleError) {
        sendJson(res, 400, { error: '予定の色カテゴリ更新に失敗しました。' });
        return;
      }
      ...
      if (error) {
        sendJson(res, 400, { error: '色カテゴリの削除に失敗しました。' });
        return;
      }
    ...
    if (error) {
      sendJson(res, 400, { error: '色カテゴリの保存に失敗しました。' });
      return;
    }
